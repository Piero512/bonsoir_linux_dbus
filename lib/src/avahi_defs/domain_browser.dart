...
class AvahiDomainBrowserItemNew extends DBusSignal{
  ...
  AvahiDomainBrowserItemNew(DBusSignal signal) : super(signal.sender, signal.path, signal.interface, signal.member, signal.values);
}
...
class AvahiDomainBrowserItemRemove extends DBusSignal{
  ...
  AvahiDomainBrowserItemRemove(DBusSignal signal) : super(signal.sender, signal.path, signal.interface, signal.member, signal.values);
}
...
class AvahiDomainBrowserFailure extends DBusSignal{
  ...
  AvahiDomainBrowserFailure(DBusSignal signal) : super(signal.sender, signal.path, signal.interface, signal.member, signal.values);
}

/// Signal data for org.freedesktop.Avahi.DomainBrowser.AllForNow.
class AvahiDomainBrowserAllForNow extends DBusSignal{

  AvahiDomainBrowserAllForNow(DBusSignal signal) : super(signal.sender, signal.path, signal.interface, signal.member, signal.values);
}
...
class AvahiDomainBrowserCacheExhausted extends DBusSignal{
  ...
  AvahiDomainBrowserCacheExhausted(DBusSignal signal) : super(signal.sender, signal.path, signal.interface, signal.member, signal.values);
}

class AvahiDomainBrowser extends DBusRemoteObject {
  AvahiDomainBrowser(DBusClient client, String destination, {DBusObjectPath path = const DBusObjectPath.unchecked('null')}) : super(client, destination, path);
  ...
  Stream<AvahiDomainBrowserItemNew> subscribeItemNew() async* {
    ...
    await for (var signal in signals) {
      if (signal.values.length == 4 && signal.values[0].signature == DBusSignature('i') && signal.values[1].signature == DBusSignature('i') && signal.values[2].signature == DBusSignature('s') && signal.values[3].signature == DBusSignature('u')) {
