...
class AvahiAddressResolverFound extends DBusSignal{
  ...
  AvahiAddressResolverFound(DBusSignal signal) : super(signal.sender, signal.path, signal.interface, signal.member, signal.values);
}
...
class AvahiAddressResolverFailure extends DBusSignal{
  ...
  AvahiAddressResolverFailure(DBusSignal signal) : super(signal.sender, signal.path, signal.interface, signal.member, signal.values);
}

class AvahiAddressResolver extends DBusRemoteObject {
  AvahiAddressResolver(DBusClient client, String destination, {DBusObjectPath path = const DBusObjectPath.unchecked('null')}) : super(client, destination, path);
  ...
  Stream<AvahiAddressResolverFound> subscribeFound() async* {
    ...
    await for (var signal in signals) {
      if (signal.values.length == 6 && signal.values[0].signature == DBusSignature('i') && signal.values[1].signature == DBusSignature('i') && signal.values[2].signature == DBusSignature('i') && signal.values[3].signature == DBusSignature('s') && signal.values[4].signature == DBusSignature('s') && signal.values[5].signature == DBusSignature('u')) {
