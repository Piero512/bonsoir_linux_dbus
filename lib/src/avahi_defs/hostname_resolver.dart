...
class OrgFreedesktopDBusIntrospectable extends DBusRemoteObject {
  ...
  Stream<OrgFreedesktopDBusIntrospectableFound> subscribeFound() async* {
    ...
    await for (var signal in signals) {
      if (signal.values.length == 6 && signal.values[0].signature == DBusSignature('i') && signal.values[1].signature == DBusSignature('i') && signal.values[2].signature == DBusSignature('s') && signal.values[3].signature == DBusSignature('i') && signal.values[4].signature == DBusSignature('s') && signal.values[5].signature == DBusSignature('u')) {
