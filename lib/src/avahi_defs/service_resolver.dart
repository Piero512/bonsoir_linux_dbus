...
class AvahiServiceResolverFound extends DBusSignal{
  ...
  AvahiServiceResolverFound(DBusSignal signal) : super(signal.sender, signal.path, signal.interface, signal.member, signal.values);
}
...
class AvahiServiceResolverFailure extends DBusSignal{
  ...
  AvahiServiceResolverFailure(DBusSignal signal) : super(signal.sender, signal.path, signal.interface, signal.member, signal.values);
}

class AvahiServiceResolver extends DBusRemoteObject {
  AvahiServiceResolver(DBusClient client, String destination, {DBusObjectPath path = const DBusObjectPath.unchecked('null')}) : super(client, destination, path);
  ...
  Stream<AvahiServiceResolverFound> subscribeFound() async* {
    ...
    await for (var signal in signals) {
      if (signal.values.length == 11 && signal.values[0].signature == DBusSignature('i') && signal.values[1].signature == DBusSignature('i') && signal.values[2].signature == DBusSignature('s') && signal.values[3].signature == DBusSignature('s') && signal.values[4].signature == DBusSignature('s') && signal.values[5].signature == DBusSignature('s') && signal.values[6].signature == DBusSignature('i') && signal.values[7].signature == DBusSignature('s') && signal.values[8].signature == DBusSignature('q') && signal.values[9].signature == DBusSignature('aay') && signal.values[10].signature == DBusSignature('u')) {
